...
pub async fn App<G: Html>(ctx: Scope<'_>) -> View<G> {
    ...
    window_event_listener_2(
        ...
        "resize",
        Box::new(|| {
            // TODO 这里注意，要使用 move 将 ctx 所有权移交给监听事件函数，否则 ctx 结束时
...
1.所以 add_event_listener_with_callback 的参数必须是 'static 的生命周期，否则会发生悬空指针。事件通知函数推出后就会被回收。
...
由于监听事件的生命周期等价于 'static，而 ctx 的生命周期是一个普通的生命周期，监听事件中又需要使用到 ctx，ctx 是一个引用，所以会发生悬空指针。
